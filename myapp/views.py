...
from django.db.models import Sum, Q, Avg, Subquery, IntegerField, F
...
from .models import (
    ...
    ProjectRiskAssessment, ProjectRequirement, ProjectChronology
)
from .risk import assess_project_delay_risk, prediction_evidence, refresh_delay_risks
from .forms import MessageForm
from .notification_utils import (
    dashboard_url_for_user,
    normalized_notification_link_for_user,
...
BILIRAN_MUNICIPALITY_COORDS = {
    'Almeria': {'lat': 11.6200, 'lng': 124.3840},
    'Biliran': {'lat': 11.5680, 'lng': 124.4750},
    'Cabucgayan': {'lat': 11.4710, 'lng': 124.5740},
    'Caibiran': {'lat': 11.5710, 'lng': 124.5820},
    'Culaba': {'lat': 11.6570, 'lng': 124.5420},
    'Kawayan': {'lat': 11.6860, 'lng': 124.4610},
    'Maripipi': {'lat': 11.7850, 'lng': 124.3440},
    'Naval': {'lat': 11.5600, 'lng': 124.3970},
}


def parse_amount_input(value, *, allow_blank=False, default=Decimal('0')):
    """Accept form amounts with commas, peso signs, or empty optional values."""
    if value is None:
        return None if allow_blank else default
    normalized = str(value).strip()
    if normalized.lower() in {'', 'none', 'null', 'undefined', 'nan'}:
        return None if allow_blank else default
    normalized = re.sub(r'(?i)php|₱|,|\s+', '', normalized)
    if not re.fullmatch(r'-?\d+(\.\d+)?', normalized):
        raise InvalidOperation
    return Decimal(normalized)


def _build_table_pdf_response(filename, report_title, table_data, column_widths=None):
    """Generate a reports-style DOST PDF containing table data only."""
    ...
    for row in table_data:
        normalized_rows.append([
            Paragraph(str(cell), wrap_style) if isinstance(cell, str) and len(cell) > 18 else cell
            for cell in row
        ])
    ...
    def add_dost_header(canvas, doc):
        ...
        try:
            ...
            if os.path.exists(logo_path):
                canvas.drawImage(ImageReader(logo_path), 30, doc.pagesize[1] - 60, width=50, height=35, mask='auto')
        except Exception:
            pass
        ...
        canvas.drawCentredString(page_width / 2, doc.pagesize[1] - 40, title_text)
        canvas.setFont('Helvetica', 9)
        canvas.drawCentredString(page_width / 2, doc.pagesize[1] - 53, 'Provincial Science and Technology Center - Biliran')
        canvas.setFont('Helvetica', 7)
        canvas.drawRightString(page_width - 30, doc.pagesize[1] - 40, f"Generated: {datetime.now().strftime('%B %d, %Y %I:%M %p')}")
        canvas.restoreState()

    frame = Frame(24, 28, doc.pagesize[0] - 48, doc.pagesize[1] - 92, id='normal')
    doc.addPageTemplates([PageTemplate(id='taskpro_table_template', frames=frame, onPage=add_dost_header)])

    table = Table(normalized_rows, colWidths=column_widths, repeatRows=1)
    ...
    doc.build([Paragraph(report_title, title_style), table])
    return response


def has_meaningful_project_coordinate(latitude, longitude, anchor=None):
    """Treat hand-picked Biliran coordinates as exact, but ignore generic defaults."""
    ...
    if not (11.35 <= lat <= 11.85 and 124.25 <= lng <= 124.70):
        return False
    if abs(lat - 11.55) < 0.0002 and abs(lng - 124.47) < 0.0002:
        return False
    if abs((lng - lat) - 112.9) < 0.0005:
        return False
    if anchor and abs(lat - anchor['lat']) < 0.0002 and abs(lng - anchor['lng']) < 0.0002:
        return False
    return True


def scattered_biliran_coordinate(municipality, seed, latitude=None, longitude=None):
    """Return exact saved coordinates, or deterministic municipality-level display coordinates."""
    key = (municipality or '').strip()
    anchor = BILIRAN_MUNICIPALITY_COORDS.get(key)
    if has_meaningful_project_coordinate(latitude, longitude, anchor):
    ...
    angle = math.radians((seed_value * 137.508) % 360)
    radius = 0.0045 + ((seed_value % 7) * 0.0017)
    lat = anchor['lat'] + math.sin(angle) * radius
    lng = anchor['lng'] + math.cos(angle) * radius * 0.85
    lat = min(max(lat, 11.43), 11.80)
    lng = min(max(lng, 124.33), 124.62)
...
def _conversation_counts(conversations):
    return {
        'all': len(conversations),
        'inbox': sum(1 for conversation in conversations if conversation['latest_message'].recipient_id == conversation['viewer_id']),
        'sent': sum(1 for conversation in conversations if conversation['latest_message'].sender_id == conversation['viewer_id']),
    }


def _forms_url_for_user(user):
    role = role_key(user)
    if role == 'administrator':
        return reverse('administrator_forms_url')
    if role == 'staff':
        return reverse('staff_forms_url')
    if role == 'beneficiary':
        return reverse('beneficiary_forms_url')
    return reverse('proponent_forms_url')


def _form_file_exists(form_template):
    if not form_template.file:
        return False
    try:
        return form_template.file.storage.exists(form_template.file.name)
    except Exception:
        return False


def _attach_form_file_state(forms):
    for form_template in forms:
        form_template.file_available = _form_file_exists(form_template)
    return forms
...
def _local_assistant_quick_answer(question):
    normalized = ' '.join((question or '').lower().split())
    if not normalized:
        return None

    project_terms = {'project', 'projects'}
    municipality_terms = {'municipality', 'municipalities', 'mun', 'town', 'towns'}
    status_terms = {'status', 'statuses', 'progress'}
    budget_terms = {'budget', 'budgets', 'fund', 'funds', 'funding'}
    task_terms = {'task', 'tasks', 'overdue', 'pending'}
    proposal_terms = {'proposal', 'proposals'}
    tokens = set(re.findall(r'[a-z0-9]+', normalized))

    if tokens & project_terms and tokens & municipality_terms:
        rows = (
            Project.objects.exclude(Q(mun__isnull=True) | Q(mun=''))
            .values('mun')
            .annotate(total=Count('id'), total_cost=Sum('total_project_cost'), released=Sum('total_funds_released'))
            .order_by('-total', 'mun')[:12]
        )
        if not rows:
            return 'No project municipality data is available yet.'
        total_projects = sum(row['total'] for row in rows)
        lines = [f'Projects by municipality ({total_projects} shown in top municipalities):']
        for row in rows:
            lines.append(
                f"- {row['mun']}: {row['total']} project(s), total cost PHP {_compact_money(row['total_cost'])}, released PHP {_compact_money(row['released'])}"
            )
        return '\n'.join(lines)

    if tokens & project_terms and tokens & status_terms:
        rows = Project.objects.values('status').annotate(total=Count('id')).order_by('-total', 'status')
        if not rows:
            return 'No project status data is available yet.'
        lines = ['Projects by status:']
        for row in rows:
            lines.append(f"- {row['status'] or 'Unspecified'}: {row['total']}")
        return '\n'.join(lines)

    if tokens & budget_terms:
        totals = Project.objects.aggregate(
            total_cost=Sum('total_project_cost'),
            released=Sum('total_funds_released'),
            liquidated=Sum('amount_liquidated'),
        )
        by_fund = (
            Project.objects.exclude(Q(fund_source__isnull=True) | Q(fund_source=''))
            .values('fund_source')
            .annotate(total=Count('id'), total_cost=Sum('total_project_cost'))
            .order_by('-total_cost')[:8]
        )
        lines = [
            'Budget and funding snapshot:',
            f"- Total project cost: PHP {_compact_money(totals['total_cost'])}",
            f"- Total funds released: PHP {_compact_money(totals['released'])}",
            f"- Amount liquidated: PHP {_compact_money(totals['liquidated'])}",
        ]
        if by_fund:
            lines.append('Top fund sources:')
            for row in by_fund:
                lines.append(f"- {row['fund_source']}: {row['total']} project(s), PHP {_compact_money(row['total_cost'])}")
        return '\n'.join(lines)

    if tokens & task_terms:
        rows = Task.objects.values('status').annotate(total=Count('id')).order_by('-total', 'status')
        overdue_count = Task.objects.filter(due_date__lt=timezone.now().date()).exclude(status='completed').count()
        lines = [f'Task snapshot: {overdue_count} overdue unfinished task(s).']
        for row in rows:
            lines.append(f"- {row['status'] or 'Unspecified'}: {row['total']}")
        return '\n'.join(lines)

    if tokens & proposal_terms:
        rows = Proposal.objects.values('status').annotate(total=Count('id')).order_by('-total', 'status')
        if not rows:
            return 'No proposal data is available yet.'
        lines = ['Proposals by status:']
        for row in rows:
            lines.append(f"- {row['status'] or 'Unspecified'}: {row['total']}")
        return '\n'.join(lines)
...
def notification_redirect_view(request, pk):
    ...
    if notif.category == 'delay_risk' and role_key(request.user) == 'administrator':
        project_id = None
        link = notif.link or ''
        match = re.search(r'/projects/(\d+)/?', str(link))
        if match:
            project_id = match.group(1)
        else:
            parsed_link = urlparse(str(link))
            query_params = parse_qs(parsed_link.query)
            project_values = query_params.get('project') or query_params.get('project_id')
            if project_values:
                project_id = project_values[0]

        if project_id:
            forecast_link = f"{reverse('administrator_forecast_url')}?level=all&notify=0&project={project_id}#forecast-project-{project_id}"
            return redirect(forecast_link)

    return redirect(normalized_notification_link_for_user(request.user, notif.link))
...
def _build_notification_payload(user):
    ...
    for notif in notifications:
        notifications_data.append({
            ...
            'link': normalized_notification_link_for_user(user, notif.link),
            'click_url': reverse('notification_redirect', args=[notif.id]),
        })
...
def administrator_dashboard_view(request):
    ...
    for p in projects_qs:
        ...
        lat, lng = scattered_biliran_coordinate(municipality, p.id, p.latitude, p.longitude)
    ...
    refresh_delay_risks(notify=False, limit=50)
    delay_risk_projects = ProjectRiskAssessment.objects.select_related('project').filter(
        level__in=['at_risk', 'likely_delayed', 'already_delayed']
    )[:5]
    ...
    context = {
        ...
        'delay_risk_count': ProjectRiskAssessment.objects.filter(level__in=['at_risk', 'likely_delayed', 'already_delayed']).count(),
...
def assistant_ai_chat_view(request):
    ...
    quick_answer = _local_assistant_quick_answer(question)
    if quick_answer:
        return JsonResponse({'ok': True, 'answer': quick_answer, 'source': 'local'})
...
def administrator_budgets_view(request):
    ...
    raw_selected_fund = normalize_fund_label(request.GET.get('fund', 'all'))
    selected_fund = 'all' if raw_selected_fund.lower() in {'', 'all'} else raw_selected_fund

    budget_funds = {
        budget_id: normalize_fund_label(fund_source)
        for budget_id, fund_source in Budget.objects.values_list('id', 'fund_source')
    }
    project_funds = {
        project_id: normalize_fund_label(fund_source)
        for project_id, fund_source in Project.objects.values_list('id', 'fund_source')
    }
    all_fund_sources = sorted({
        label for label in [*budget_funds.values(), *project_funds.values()]
        if label and label.lower() != 'all'
    })

    if selected_fund != 'all' and selected_fund not in all_fund_sources:
        selected_fund = 'all'

    if selected_fund == 'all':
        budgets = Budget.objects.all().order_by('-date_created')
        project_budget_qs = Project.objects.select_related('budget').all()
    else:
        budget_ids = [pk for pk, fund in budget_funds.items() if fund == selected_fund]
        project_ids = [pk for pk, fund in project_funds.items() if fund == selected_fund]
        budgets = Budget.objects.filter(id__in=budget_ids).order_by('-date_created')
        project_budget_qs = Project.objects.select_related('budget').filter(id__in=project_ids)

    excel_equipment_qs = ProjectEquipment.objects.select_related(
        'project', 'budget_allocation__budget', 'budget_allocation__equipment_item'
    )
    if selected_fund != 'all':
        excel_equipment_qs = excel_equipment_qs.filter(
            Q(project_id__in=list(project_budget_qs.values_list('id', flat=True))) |
            Q(budget_allocation__budget_id__in=list(budgets.values_list('id', flat=True)))
        )

    equipment_costs_by_project = {
        item['project_id']: item['total'] or Decimal('0.00')
        for item in excel_equipment_qs.exclude(project_id__isnull=True)
        .values('project_id')
        .annotate(total=Sum('actual_total_cost'))
    }

    fund_source_data = {}
    fiscal_year_data = {}

    def add_chart_amount(container, key, total_amount, released_amount):
        key = normalize_fund_label(key) or 'Unspecified'
        total_amount = money(total_amount)
        released_amount = money(released_amount)
        if total_amount <= 0 and released_amount > 0:
            total_amount = released_amount
        if released_amount > total_amount and total_amount > 0:
            released_amount = total_amount
        remaining_amount = max(total_amount - released_amount, Decimal('0.00'))
        if key not in container:
            container[key] = {'total': 0.0, 'remaining': 0.0, 'spent': 0.0}
        container[key]['total'] += float(total_amount)
        container[key]['spent'] += float(released_amount)
        container[key]['remaining'] += float(remaining_amount)

    for project in project_budget_qs:
        source = normalize_fund_label(project.fund_source) or normalize_fund_label(project.budget.fund_source if project.budget else '')
        total_amount = money(project.total_project_cost) or money(project.funds) or equipment_costs_by_project.get(project.id, Decimal('0.00'))
        released_amount = money(project.total_funds_released) or money(project.amount_liquidated)
        year = project.year or (project.project_start.year if project.project_start else None) or (project.date_of_release.year if project.date_of_release else None) or 'Unspecified'
        add_chart_amount(fund_source_data, source, total_amount, released_amount)
        add_chart_amount(fiscal_year_data, str(year), total_amount, released_amount)

    if not any(item['total'] for item in fund_source_data.values()):
        for budget in budgets:
            source = normalize_fund_label(budget.fund_source)
            add_chart_amount(fund_source_data, source, budget.total_amount, budget.total_amount - budget.remaining_amount)
            add_chart_amount(fiscal_year_data, str(budget.fiscal_year), budget.total_amount, budget.total_amount - budget.remaining_amount)

    total_budget = Decimal(str(sum(item['total'] for item in fund_source_data.values())))
    total_spent = Decimal(str(sum(item['spent'] for item in fund_source_data.values())))
    total_remaining = max(total_budget - total_spent, Decimal('0.00'))
    ...
    actual_equipment_cost = excel_equipment_qs.aggregate(total=Sum('actual_total_cost'))['total'] or Decimal('0.00')
    equipment_items_count = excel_equipment_qs.count()
    missing_supplier_count = excel_equipment_qs.filter(Q(supplier_name__isnull=True) | Q(supplier_name='')).count()
    liquidated_projects = Project.objects.filter(status_of_liquidation__icontains='liquidated')
    unliquidated_projects = Project.objects.filter(status_of_liquidation__icontains='unliquidated')
    if selected_fund != 'all':
        filtered_project_ids = list(project_budget_qs.values_list('id', flat=True))
        liquidated_projects = liquidated_projects.filter(id__in=filtered_project_ids)
        unliquidated_projects = unliquidated_projects.filter(id__in=filtered_project_ids)

    equipment_recent = excel_equipment_qs.order_by('-date_acquired', '-delivery_date')[:8]
    ...
    for budget in budgets:
        ...
        for eq_alloc in equipment_allocations:
            allocation_details.append({
                ...
                'delivered_quantity': eq_alloc.delivered_quantity,
        ...
        for project in projects:
            allocation_details.append({
                ...
                'liquidation_status': project.status_of_liquidation or 'N/A',
                'risk_status': project.delay_risk_status or 'not_checked',
            })
        ...
        budget_allocations.append({
            ...
            'fiscal_year': budget.fiscal_year,
                ...
                'equipment_items': equipment_allocations.count(),
                'delivered_items': sum(a.delivered_quantity for a in equipment_allocations),
    ...
    context = {
        ...
        'actual_equipment_cost': actual_equipment_cost,
        'equipment_items_count': equipment_items_count,
        'missing_supplier_count': missing_supplier_count,
        'liquidated_projects_count': liquidated_projects.count(),
        'unliquidated_projects_count': unliquidated_projects.count(),
        'equipment_recent': equipment_recent,
    }
...
def administrator_proposals_update_view(request, pk):
    ...
    if request.method == 'POST':
        ...
        try:
            proposal.proposed_amount = parse_amount_input(request.POST.get('proposed_amount'), default=Decimal('0'))
            proposal.approved_amount = parse_amount_input(request.POST.get('approved_amount'), allow_blank=True)
...
def administrator_proposals_approve_view(request, pk):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            receivers_set = set()
            if proposal.submitted_by and role_key(proposal.submitted_by) != 'administrator':
                receivers_set.add(proposal.submitted_by)
            if proposal.proponent and role_key(proposal.proponent) != 'administrator':
                receivers_set.add(proposal.proponent)
            if proposal.beneficiary and role_key(proposal.beneficiary) != 'administrator':
            ...
            for receiver in receivers_set:
                Notification.objects.create(
                    ...
                    category='proposal',
                    link=notification_url_for_user(receiver, 'proposals')
                )
...
def administrator_proposals_decline_view(request):
    if request.method == 'POST':
        ...
        if proposal.proponent and role_key(proposal.proponent) != 'administrator' and proposal.proponent not in receivers:
...
def administrator_projects_view(request):
    projects = list(Project.objects.select_related('proposal', 'budget', 'project_leader').all())
    draft_projects = [project for project in projects if (project.status or '').strip().lower() == 'draft']
    project_risk_map = {
        assessment.project_id: assessment
        for assessment in ProjectRiskAssessment.objects.filter(project_id__in=[p.id for p in projects])
    }
    for project in projects:
        assessment = project_risk_map.get(project.id)
        project.forecast_actual_progress = assessment.actual_progress if assessment else prediction_evidence(project)['actual_progress']
        project.forecast_expected_progress = assessment.expected_progress if assessment else prediction_evidence(project)['expected_progress']
        project.forecast_risk_level = assessment.get_level_display() if assessment else 'Not assessed'
    # Get budgets that are available for allocation (includes legacy 'active' status)
    budgets = Budget.objects.filter(status__in=['available', 'partially_allocated', 'active'])
    ...
    projects_data = []
    for p in projects:
        display_lat, display_lng = scattered_biliran_coordinate(p.mun, p.id, p.latitude, p.longitude)
        is_exact_location = has_meaningful_project_coordinate(
            p.latitude,
            p.longitude,
            BILIRAN_MUNICIPALITY_COORDS.get((p.mun or '').strip())
        )
        p.display_latitude = display_lat
        p.display_longitude = display_lng
        p.location_accuracy = 'Exact pinned coordinate' if is_exact_location else 'Municipality-level estimate'
...
@login_required
def administrator_forecast_view(request):
    from django.core.paginator import Paginator
    from django.db import OperationalError

    if role_key(request.user) != 'administrator':
        messages.error(request, 'You do not have permission to access the forecast module.')
        return redirect(dashboard_url_for_user(request.user))

    should_refresh = request.GET.get('refresh') == '1' or request.GET.get('notify') == '1'
    if should_refresh:
        try:
            refresh_delay_risks(notify=request.GET.get('notify') == '1')
            messages.success(request, 'Forecast risks were refreshed successfully.')
        except OperationalError:
            messages.warning(
                request,
                'Forecast refresh was skipped because the local SQLite database is busy. '
                'The current stored forecast is still shown below.'
            )

    valid_levels = {'all', 'flagged', 'on_track', 'at_risk', 'likely_delayed', 'already_delayed'}
    selected_level = request.GET.get('level', 'flagged')
    if selected_level not in valid_levels:
        selected_level = 'flagged'

    level_priority = Case(
        When(level='already_delayed', then=Value(0)),
        When(level='likely_delayed', then=Value(1)),
        When(level='at_risk', then=Value(2)),
        When(level='on_track', then=Value(3)),
        default=Value(4),
        output_field=IntegerField(),
    )

    assessments = ProjectRiskAssessment.objects.select_related(
        'project',
        'project__project_leader',
        'project__proposal',
    ).annotate(level_priority=level_priority)

    if selected_level == 'flagged':
        assessments = assessments.exclude(level='on_track')
    elif selected_level != 'all':
        assessments = assessments.filter(level=selected_level)

    assessments = assessments.order_by(
        'level_priority',
        '-score',
        'days_remaining',
        'project__project_end',
        'project__project_title',
    )

    risk_counts = {row['level']: row['total'] for row in ProjectRiskAssessment.objects.values('level').annotate(total=Count('id'))}
    flagged_count = sum(risk_counts.get(level, 0) for level in ('at_risk', 'likely_delayed', 'already_delayed'))
    projects_total = Project.objects.count()
    highest_risk = assessments.first()
    avg_score = ProjectRiskAssessment.objects.aggregate(avg=Avg('score'))['avg'] or 0
    flagged_assessments = ProjectRiskAssessment.objects.select_related('project').exclude(level='on_track')
    overdue_flagged_count = flagged_assessments.filter(days_remaining__lt=0).count()
    behind_schedule_count = flagged_assessments.filter(actual_progress__lt=F('expected_progress')).count()
    extension_candidates_count = flagged_assessments.filter(suggested_extension_days__gt=0).count()
    avg_progress_gap = flagged_assessments.aggregate(
        gap=Avg(F('expected_progress') - F('actual_progress'))
    )['gap'] or 0
    review_counts = {
        row['review_status']: row['total']
        for row in ProjectRiskAssessment.objects.values('review_status').annotate(total=Count('id'))
    }
    due_followups_count = ProjectRiskAssessment.objects.exclude(review_status='resolved').filter(
        follow_up_date__isnull=False,
        follow_up_date__lte=timezone.localdate()
    ).count()
    reviewers = User.objects.filter(role__in=['admin', 'dost_staff'], status='active').order_by(
        'first_name', 'last_name', 'username'
    )
    top_municipalities = (
        flagged_assessments.values('project__mun')
        .annotate(total=Count('id'), avg_score=Avg('score'))
        .order_by('-total', '-avg_score')[:5]
    )

    missing_dates = Project.objects.filter(Q(project_start__isnull=True) | Q(project_end__isnull=True))
    missing_coordinates = Project.objects.filter(Q(latitude__isnull=True) | Q(longitude__isnull=True))
    missing_budgets = Project.objects.filter(
        Q(budget__isnull=True) & (Q(funds__isnull=True) | Q(funds=0)) & (Q(total_project_cost__isnull=True) | Q(total_project_cost=0))
    )
    projects_with_counts = Project.objects.annotate(
        forecast_task_count=Count('tasks', distinct=True),
        forecast_milestone_count=Count('milestones', distinct=True),
        forecast_avg_task_progress=Avg('tasks__progress_percentage'),
        forecast_avg_milestone_progress=Avg('milestones__progress_percentage'),
    )
    no_tasks = projects_with_counts.filter(forecast_task_count=0)
    no_progress_updates = projects_with_counts.filter(
        Q(forecast_task_count=0, forecast_milestone_count=0)
        | (
            Q(forecast_avg_task_progress__isnull=True) | Q(forecast_avg_task_progress=0)
        ) & (
            Q(forecast_avg_milestone_progress__isnull=True) | Q(forecast_avg_milestone_progress=0)
        )
    ).exclude(status__iexact='completed')
    incomplete_documents = Project.objects.filter(requirements__status='missing').distinct()
    data_quality_items = [
        {
            'key': 'missing-dates',
            'label': 'Missing dates',
            'count': missing_dates.count(),
            'help': 'Projects need both start and end dates for reliable risk timing.',
            'icon': 'event_busy',
            'tone': 'amber',
            'projects': missing_dates.order_by('-date_updated'),
        },
        {
            'key': 'missing-coordinates',
            'label': 'Missing coordinates',
            'count': missing_coordinates.count(),
            'help': 'Map markers need latitude and longitude.',
            'icon': 'location_off',
            'tone': 'blue',
            'projects': missing_coordinates.order_by('-date_updated'),
        },
        {
            'key': 'missing-budgets',
            'label': 'Missing budgets',
            'count': missing_budgets.count(),
            'help': 'Funding signals are weaker when project budget fields are empty.',
            'icon': 'account_balance_wallet',
            'tone': 'red',
            'projects': missing_budgets.order_by('-date_updated'),
        },
        {
            'key': 'no-tasks',
            'label': 'No tasks',
            'count': no_tasks.count(),
            'help': 'Task progress cannot be measured until work items exist.',
            'icon': 'task_alt',
            'tone': 'slate',
            'projects': no_tasks.order_by('-date_updated'),
        },
        {
            'key': 'no-progress',
            'label': 'No progress updates',
            'count': no_progress_updates.count(),
            'help': 'Risk scoring falls back to status when task and milestone progress are blank.',
            'icon': 'trending_flat',
            'tone': 'violet',
            'projects': no_progress_updates.order_by('-date_updated'),
        },
        {
            'key': 'incomplete-documents',
            'label': 'Incomplete documents',
            'count': incomplete_documents.count(),
            'help': 'Required-document records marked missing are included in risk evidence.',
            'icon': 'description',
            'tone': 'emerald',
            'projects': incomplete_documents.order_by('-date_updated'),
        },
    ]
    ...
    focus_project_id = request.GET.get('project') or request.GET.get('project_id')
    try:
        focus_project_id = int(focus_project_id) if focus_project_id else None
    except (TypeError, ValueError):
        focus_project_id = None

    page_number = request.GET.get('page')
    if focus_project_id and not page_number:
        assessment_project_ids = list(assessments.values_list('project_id', flat=True))
        if focus_project_id in assessment_project_ids:
            page_number = (assessment_project_ids.index(focus_project_id) // per_page) + 1

    paginator = Paginator(assessments, per_page)
    page_obj = paginator.get_page(page_number)
    on_track_percent = round((risk_counts.get('on_track', 0) / projects_total) * 100) if projects_total else 0
    ...
    context = {
        ...
        'on_track_count': risk_counts.get('on_track', 0),
        'at_risk_count': risk_counts.get('at_risk', 0),
        'likely_delayed_count': risk_counts.get('likely_delayed', 0),
        'already_delayed_count': risk_counts.get('already_delayed', 0),
        'avg_score': round(avg_score),
        'overdue_flagged_count': overdue_flagged_count,
        'behind_schedule_count': behind_schedule_count,
        'extension_candidates_count': extension_candidates_count,
        'avg_progress_gap': round(avg_progress_gap),
        'review_counts': review_counts,
        'review_open_count': review_counts.get('open', 0),
        'review_acknowledged_count': review_counts.get('acknowledged', 0),
        'review_in_review_count': review_counts.get('in_review', 0),
        'review_resolved_count': review_counts.get('resolved', 0),
    ...
    return render(request, 'administrator/forecast.html', context)


@login_required
@require_POST
def administrator_forecast_review_update_view(request, pk):
    if role_key(request.user) != 'administrator':
        messages.error(request, 'You do not have permission to update forecast reviews.')
        return redirect(dashboard_url_for_user(request.user))

    assessment = get_object_or_404(ProjectRiskAssessment.objects.select_related('project'), pk=pk)
    action = request.POST.get('action', '').strip()
    update_fields = []

    if action == 'acknowledge':
        assessment.review_status = 'acknowledged'
        assessment.acknowledged_by = request.user
        assessment.acknowledged_at = timezone.now()
        update_fields = ['review_status', 'acknowledged_by', 'acknowledged_at']
        message = 'Risk acknowledged.'
    elif action == 'assign':
        reviewer_id = request.POST.get('assigned_reviewer')
        reviewer = get_object_or_404(User, pk=reviewer_id, role__in=['admin', 'dost_staff'], status='active')
        assessment.assigned_reviewer = reviewer
        if assessment.review_status == 'open':
            assessment.review_status = 'in_review'
            update_fields.append('review_status')
        update_fields.append('assigned_reviewer')
        message = 'Reviewer assigned.'
    elif action == 'follow_up':
        follow_up_date = parse_date(request.POST.get('follow_up_date') or '')
        if not follow_up_date:
            messages.error(request, 'Please choose a valid follow-up date.')
            return redirect(f"{reverse('administrator_forecast_url')}?level={request.POST.get('level', 'flagged')}&notify=0&per_page={request.POST.get('per_page', '10')}")
        ...
        raw_notes = (request.POST.get('review_notes') or '').strip()
        assessment.review_notes = sanitize_string(raw_notes) if raw_notes else ''
        update_fields = ['review_notes']
        message = 'Review notes saved.'
    elif action == 'resolve':
        assessment.review_status = 'resolved'
        assessment.resolved_by = request.user
        assessment.resolved_at = timezone.now()
        update_fields = ['review_status', 'resolved_by', 'resolved_at']
        message = 'Risk marked as resolved.'
    elif action == 'reopen':
        assessment.review_status = 'open'
        assessment.resolved_by = None
        assessment.resolved_at = None
        update_fields = ['review_status', 'resolved_by', 'resolved_at']
        message = 'Risk reopened.'
    else:
        messages.error(request, 'Unknown forecast review action.')
        return redirect('administrator_forecast_url')
    ...
    AuditLog.objects.create(
        ...
        details=f"{message} Project: {assessment.project.project_title or assessment.project.project_code or assessment.project_id}",
    )
    messages.success(request, message)

    level = request.POST.get('level', 'flagged')
    per_page = request.POST.get('per_page', '10')
    page = request.POST.get('page')
    query = f"level={level}&notify=0&per_page={per_page}"
    if page:
        query += f"&page={page}"
    return redirect(f"{reverse('administrator_forecast_url')}?{query}")
...
def administrator_projects_detail_view(request, pk):
    ...
    risk_assessment = assess_project_delay_risk(project, save=True)
    ...
    context = {
        ...
        'chronology': project.chronology.all()[:8],
        'equipment_deliveries': project.equipment_deliveries.select_related('budget_allocation__equipment_item').all(),
    }
...
@login_required
@require_POST
def administrator_project_issue_extension_request_view(request, pk):
    project = get_object_or_404(Project, pk=pk)
    assessment = assess_project_delay_risk(project, save=True)
    evidence = prediction_evidence(project)
    days = request.POST.get('requested_extension_days') or assessment.suggested_extension_days or 30
    try:
        days = max(1, int(days))
    except (TypeError, ValueError):
        days = 30
    ...
    reason = request.POST.get('reason') or (
        "System-generated delay-risk extension request.\n\n"
        f"Project: {project.project_title or project.project_code or project.pk}\n"
        f"Risk level: {assessment.get_level_display()} ({assessment.score}%).\n"
        f"Actual progress: {evidence['actual_progress']}%; expected progress: {evidence['expected_progress']}%; gap: {evidence['progress_gap']}%.\n"
        f"Days remaining: {evidence['days_remaining'] if evidence['days_remaining'] is not None else 'timeline incomplete'}.\n"
        f"Suggested extension: {days} day(s).\n"
        f"Evidence: {evidence['overdue_tasks']} overdue task(s), {evidence['missing_requirements']} missing requirement(s), "
        f"{evidence['task_count']} task(s), {evidence['milestone_count']} milestone(s).\n\n"
        "Flag reasons:\n"
        + "\n".join(f"- {reason}" for reason in assessment.reasons)
    )

    proponent = project.project_leader or request.user
    extension_request = ExtensionRequest.objects.create(
        ...
        status='pending',
        remarks='Generated from TaskPro rule-based deadline risk assessment.'
    )
    ...
    AuditLog.objects.create(
        ...
        object_id=str(extension_request.pk),
        details=f'Issued from project delay-risk assessment for {project.project_code or project.project_title}.',
        new_data={
    ...
    messages.success(request, 'Extension request issued from the project delay-risk assessment.')
...
def administrator_projects_add_view(request):
    if request.method == 'POST':
        ...
        project_status = 'Draft' if save_mode == 'draft' else request.POST.get('status')
        ...
        try:
            ...
            if save_mode == 'draft':
                messages.success(request, 'Project draft saved. You can continue editing it from the Draft Projects section.')
            else:
                messages.success(request, 'Project added successfully.')
        except Exception as e:
    ...
    if request.method == 'POST':
        ...
        if assigned_to:
            Notification.objects.create(
                ...
                category='task',
                link=notification_url_for_user(assigned_to, 'tasks')
            )
    ...
    if request.method == 'POST':
        ...
        if new_assigned_to and new_assigned_to != old_assigned_to:
            Notification.objects.create(
                ...
                link=notification_url_for_user(new_assigned_to, 'tasks')
            )
    ...
    if request.method == 'POST':
        ...
        if task.assigned_to:
            Notification.objects.create(
                ...
                link=notification_url_for_user(task.assigned_to, 'tasks')
            )
...
@login_required
def export_proposals_table_pdf(request):
    if role_key(request.user) != 'administrator':
        return HttpResponseForbidden('You do not have permission to export proposal records.')

    proposals = Proposal.objects.select_related('proponent', 'beneficiary', 'budget', 'project').order_by('-submission_date')
    table_data = [[
        'No.', 'Proposal Title', 'Proponent', 'Beneficiary', 'Location',
        'Proposed Amount', 'Approved Amount', 'Status', 'Documents', 'Linked Project'
    ]]

    for index, proposal in enumerate(proposals, start=1):
        location = ', '.join(part for part in [proposal.municipality, proposal.province] if part) or (proposal.location or '-')
        table_data.append([
            index,
            proposal.title or '-',
            proposal.proponent.full_name() if proposal.proponent else '-',
            proposal.beneficiary.full_name() if proposal.beneficiary else '-',
            location,
            f"PHP {proposal.proposed_amount:,.2f}" if proposal.proposed_amount is not None else 'PHP 0.00',
            f"PHP {proposal.approved_amount:,.2f}" if proposal.approved_amount is not None else '-',
            proposal.get_status_display(),
            proposal.documents.count(),
            proposal.project.project_title if getattr(proposal, 'project', None) else '-',
        ])

    return _build_table_pdf_response(
        'taskpro_proposals_table.pdf',
        'Project Proposals Table',
        table_data,
        [24, 140, 82, 82, 70, 64, 64, 58, 42, 104],
    )


@login_required
def export_projects_table_pdf(request):
    if role_key(request.user) != 'administrator':
        return HttpResponseForbidden('You do not have permission to export project records.')

    projects = Project.objects.select_related('proposal', 'budget', 'project_leader').order_by('project_code', 'project_title')
    table_data = [[
        'Code', 'Project Title', 'Agency', 'Program', 'Type', 'Municipality',
        'Status', 'Progress', 'Funds', 'Start', 'End'
    ]]
    ...
    for project in projects:
        ...
        table_data.append([
            ...
            project.mun or '-',
            project.status or '-',
            f"{evidence.get('actual_progress', 0)}%",
            f"PHP {(project.funds or Decimal('0')):,.2f}",
            project.project_start.strftime('%b %d, %Y') if project.project_start else '-',
            project.project_end.strftime('%b %d, %Y') if project.project_end else '-',
        ])

    return _build_table_pdf_response(
        'taskpro_projects_table.pdf',
        'Projects Table',
        table_data,
        [62, 150, 78, 56, 72, 58, 56, 44, 70, 58, 58],
    )
...
def administrator_extension_requests_view(request):
    ...
    extension_requests = ExtensionRequest.objects.select_related(
        'proposal', 'project', 'proponent', 'proposal__project'
    ).prefetch_related(
        'project__tasks', 'project__requirements', 'project__chronology'
    ).order_by('-date_submitted')

    for extension_request in extension_requests:
        project = extension_request.project or getattr(extension_request.proposal, 'project', None)
        extension_request.prediction_project = project
        if project:
            try:
                risk_assessment = project.risk_assessment
                extension_request.risk_level_display = risk_assessment.get_level_display()
                extension_request.risk_score = risk_assessment.score
                extension_request.risk_reasons = risk_assessment.reasons
                extension_request.risk_suggested_days = risk_assessment.suggested_extension_days
            except ProjectRiskAssessment.DoesNotExist:
                risk_assessment = assess_project_delay_risk(project, save=False)
                extension_request.risk_level_display = risk_assessment['level'].replace('_', ' ').title()
                extension_request.risk_score = risk_assessment['score']
                extension_request.risk_reasons = risk_assessment['reasons']
                extension_request.risk_suggested_days = risk_assessment['suggested_extension_days']
            extension_request.risk_assessment = risk_assessment
            extension_request.prediction_evidence = prediction_evidence(project)
        else:
            extension_request.risk_assessment = None
            extension_request.risk_level_display = None
            extension_request.risk_score = None
            extension_request.risk_reasons = []
            extension_request.risk_suggested_days = None
            extension_request.prediction_evidence = None
...
def administrator_extension_requests_bulk_approve_view(request):
    ...
    for request_id in request_ids:
        try:
            ...
            project = ext_request.project or getattr(ext_request.proposal, 'project', None)
            if project and approved_days:
                if project.project_end:
                    project.project_end += timedelta(days=int(approved_days))
                if project.date_of_completion:
                    project.date_of_completion += timedelta(days=int(approved_days))
                project.extension_date = f"{approved_days} day(s) approved on {timezone.localdate().isoformat()}"
                project.save(update_fields=['project_end', 'date_of_completion', 'extension_date', 'date_updated'])
...
def administrator_extension_requests_approve_view(request, pk):
    ...
    if request.method == 'POST':
        ...
        Notification.objects.create(
            ...
            message=f'Your extension request for "{extension_request.subject_title}" has been approved for {approved_days} days',
            link=notification_url_for_user(extension_request.proponent, 'extension-requests')
        )
...
def administrator_extension_requests_reject_view(request, pk):
    ...
    if request.method == 'POST':
        ...
        Notification.objects.create(
            sender=request.user,
            receiver=extension_request.proponent,
            message=f'Your extension request for "{extension_request.subject_title}" has been rejected',
            link=notification_url_for_user(extension_request.proponent, 'extension-requests')
        )
        ...
        try:
            ...
            for admin in admins:
                Notification.objects.create(
                    ...
                    link=notification_url_for_user(admin, 'proposals')
                )
            ...
            if processed_by:
                Notification.objects.create(
                    ...
                    link=notification_url_for_user(processed_by, 'proposals')
                )
            ...
            if beneficiary:
                Notification.objects.create(
                    ...
                    link=notification_url_for_user(beneficiary, 'proposals')
                )
            # -------------------------
        ...
        try:
            ...
            if proposal.processed_by:
                Notification.objects.create(
                    ...
                    message=f"You have been assigned to process proposal: {proposal.title}",
                    link=notification_url_for_user(proposal.processed_by, 'proposals')
                )
            ...
            if proposal.beneficiary:
                Notification.objects.create(
                    ...
                    message=f"You are listed as beneficiary for proposal: {proposal.title}",
                    link=notification_url_for_user(proposal.beneficiary, 'proposals')
                )
            ...
            if proposal.submitted_by != proposal.processed_by:
                Notification.objects.create(
                    ...
                    link=notification_url_for_user(proposal.submitted_by, 'proposals')
                )
    ...
    if request.method == 'POST':
        ...
        for admin in admin_users:
            Notification.objects.create(
                ...
                link=notification_url_for_user(admin, 'tasks')
            )
        ...
        if proposal and proposal.processed_by:
            Notification.objects.create(
                ...
                link=notification_url_for_user(proposal.processed_by, 'tasks')
            )
        # -------------------------
...
def staff_reports_view(request):
    ...
    # Total Budget
    # -----------------------------
    total_budget = sum((budget.total_amount for budget in budget_queryset), Decimal('0.00'))
...
def _update_role_task_status(request, task_queryset, redirect_url_name, role_label):
    if request.method != 'POST':
    ...
    for admin in User.objects.filter(role='admin'):
        Notification.objects.create(
            sender=request.user,
            receiver=admin,
            message=f"{role_label} updated task '{task.title}' to '{task.status}'.",
            link=notification_url_for_user(admin, 'tasks')
        )
...
@login_required
def proponent_task_edit_view(request):
    task_queryset = Task.objects.select_related('project', 'project__proposal').filter(
        project__proposal__processed_by=request.user
    )
    return _update_role_task_status(request, task_queryset, 'proponent_task_list_url', 'Proponent')
...
def proponent_reports_view(request):
    ...
    for budget in budgets:
        ...
        for project in projects:
            ...
            for task in project.tasks.all():
                location_name = task.location_name or ', '.join(
                    value for value in [project.mun, project.province] if value
                ) or "N/A"
    ...
    context = {
        ...
        'end_date': end_date,
        'selected_status': selected_status,
        'report_year': int(selected_year) if str(selected_year or '').isdigit() else current_year,
    }
...
def proponent_extension_requests_add_view(request):
    ...
    if request.method == 'POST':
        ...
        admin_users = User.objects.filter(Q(role='admin') | Q(role='administrator') | Q(is_superuser=True)).distinct()
        for admin in admin_users:
            Notification.objects.create(
                sender=request.user,
                receiver=admin,
                message=f'New extension request submitted by {request.user.get_full_name()} for proposal "{proposal.title}"',
                link=notification_url_for_user(admin, 'extension-requests')
            )
...
@login_required
def beneficiary_proposals_add_view(request):
    if request.method == 'POST':
        title = request.POST.get('title')
        description = request.POST.get('description')
        proposed_amount = request.POST.get('proposed_amount') or 0
        ...
        processed_by_id = request.POST.get('processed_by')
        processed_by = User.objects.filter(pk=processed_by_id, role='proponent').first() if processed_by_id else None
        ...
        try:
            ...
            AuditLog.objects.create(
                ...
                new_data={
                    ...
                    "budget": budget.fund_source if budget else None,
                    "processed_by": processed_by.get_full_name() if processed_by else None,
                    "beneficiary": request.user.get_full_name(),
                    "document": proposal.document.url if proposal.document else None,
                }
            )
            ...
            for admin in User.objects.filter(role='admin'):
                Notification.objects.create(
                    sender=request.user,
                    receiver=admin,
                    message=f"New beneficiary proposal submitted: {proposal.title}",
                    link=notification_url_for_user(admin, 'proposals')
                )
    ...
    return redirect('beneficiary_proposals_url')
...
@login_required
def beneficiary_task_edit_view(request):
    task_queryset = Task.objects.select_related('project', 'project__proposal').filter(
        project__proposal__beneficiary=request.user
    )
    return _update_role_task_status(request, task_queryset, 'beneficiary_task_list_url', 'Beneficiary')
...
def administrator_forms_view(request):
    ...
    forms = _attach_form_file_state(FormTemplate.objects.select_related('uploaded_by').all().order_by('-date_uploaded'))
...
def form_download_view(request, form_id):
    ...
    if not _form_file_exists(form_template):
        messages.error(
            request,
            f'The file for "{form_template.title}" is missing from storage. Please re-upload or replace this form template.'
        )
        return redirect(_forms_url_for_user(request.user))
    ...
    response['Content-Disposition'] = f'{disposition}; filename="{form_template.title}.{ext}"'
...
def administrator_communication_hub_view(request):
    ...
    cache_key = f'admin_comm_hub_payload_{user.id}_v2'
...
def administrator_messages_view(request):
    ...
    user = request.user
    all_conversations = _build_user_conversations(user)
    conversations = list(all_conversations)
    active_box = request.GET.get('box', 'all')
    search_query = request.GET.get('search', '').strip()
    counts = _conversation_counts(all_conversations)
...
def delete_signature_api(request, signature_id):
    ...
    try:
        ...
        if role_key(request.user) != 'administrator' and signature.user != request.user:
...
def create_mention_view(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            mention_section = 'group-chats' if content_type == 'group_chat_message' else 'announcements' if content_type == 'announcement' else 'messages'
            Notification.objects.create(
                sender=request.user,
                receiver=mentioned_user,
                message=f'{request.user.first_name} {request.user.last_name} mentioned you: {message_preview}',
                category='chat' if mention_section in {'messages', 'group-chats'} else 'announcement',
                link=notification_url_for_user(mentioned_user, mention_section),
            )
