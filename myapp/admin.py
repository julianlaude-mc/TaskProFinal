...
# Simple Admin Mixin
# ------------------------
class SimpleAdminMixin:
    """Mixin to provide simplified admin views when 'simple mode' is active.

    Detection order: per-user attribute `user.simple_mode` (if present) then
    cookie `simple_mode`. This lets us support both DB-backed flags and the
    current cookie-based fallback.
    """
    def _is_simple(self, request):
        try:
            if getattr(request, 'user', None) and request.user.is_authenticated:
                if getattr(request.user, 'simple_mode', False):
                    return True
        except Exception:
            # defensive: continue to cookie fallback
            pass
    ...
    def get_list_display(self, request):
        if self._is_simple(request):
            return getattr(self, 'simple_list_display', tuple(self.list_display[:4]))
        return super().get_list_display(request)

    def get_list_filter(self, request):
        if self._is_simple(request):
            return getattr(self, 'simple_list_filter', tuple())
        return super().get_list_filter(request)

    def get_search_fields(self, request):
        if self._is_simple(request):
            return getattr(self, 'simple_search_fields', tuple())
        return super().get_search_fields(request)

    def get_fieldsets(self, request, obj=None):
        if self._is_simple(request):
            return getattr(self, 'simple_fieldsets', self.fieldsets)
...
# ------------------------
@admin.register(User)
class UserAdmin(SimpleAdminMixin, admin.ModelAdmin):
    list_display = ('username', 'full_name', 'email', 'role', 'status', 'tna_status', 'date_created')
    # Simplified view for simple mode
    simple_list_display = ('username', 'full_name', 'email', 'role')
    simple_list_filter = ('role', 'status')
    simple_search_fields = ('username', 'email')
...
class ProjectAdmin(SimpleAdminMixin, admin.ModelAdmin):
    ...
    # Simplified admin for simple mode
    simple_list_display = ('project_code', 'project_title', 'funds', 'status')
    simple_list_filter = ('status', 'year')
    simple_search_fields = ('project_title', 'project_code')
...
@admin.register(ProjectRiskAssessment)
class ProjectRiskAssessmentAdmin(admin.ModelAdmin):
    list_display = ('project', 'level', 'score', 'days_remaining', 'expected_progress', 'actual_progress', 'last_checked')
    list_filter = ('level', 'last_checked')
    search_fields = ('project__project_code', 'project__project_title')
    readonly_fields = ('last_checked',)


@admin.register(ProjectRequirement)
class ProjectRequirementAdmin(admin.ModelAdmin):
    list_display = ('project', 'name', 'category', 'status', 'date_submitted', 'date_updated')
    list_filter = ('status', 'category')
    search_fields = ('project__project_code', 'project__project_title', 'name')


@admin.register(ProjectChronology)
class ProjectChronologyAdmin(admin.ModelAdmin):
    list_display = ('project', 'event_date', 'event', 'source', 'date_created')
    list_filter = ('source', 'event_date')
    search_fields = ('project__project_code', 'project__project_title', 'event', 'remarks')
